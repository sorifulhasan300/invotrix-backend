...
const request = async (method: string, path: string, body?: any, token?: string) => {
...
const makeFormData = (sku: string, name: string, extra: Record<string, any> = {}) => {
  const validPng =
...
const run = async () => {
  ...
  assert("login as Manager returns token", () => {
    if (!managerToken) throw new Error("No manager token");
  });

  employeeToken = await loginEmployee();
  assert("login as Employee returns token", () => {
    if (!employeeToken) throw new Error("No employee token");
  });
  ...
  assert("Admin can create product (201)", () => {
    if (createAdmin.status !== 201) throw new Error(`Expected 201, got ${createAdmin.status} - ${JSON.stringify(createAdmin.data)}`);
    if (createAdmin.data.data?.sku !== "SKU-UNIQ-001") throw new Error("SKU mismatch");
  });
  ...
  assert("Manager can create product (201)", () => {
    if (createManager.status !== 201) throw new Error(`Expected 201, got ${createManager.status} - ${JSON.stringify(createManager.data)}`);
  });
  ...
  assert("Employee cannot create product (403)", () => {
    if (employeeCreate.status !== 403) throw new Error(`Expected 403, got ${employeeCreate.status}`);
  });
  ...
  assert("create without image returns 400", () => {
    if (createNoImage.status !== 400) throw new Error(`Expected 400, got ${createNoImage.status}`);
  });

  const createNoAuth = await request("POST", "/products", { name: "test" });
  assert("create without auth returns 401", () => {
    if (createNoAuth.status !== 401) throw new Error(`Expected 401, got ${createNoAuth.status}`);
  });
  ...
  assert("Admin can get all products (200)", () => {
    if (getAllAdmin.status !== 200) throw new Error(`Expected 200, got ${getAllAdmin.status}`);
    if (!Array.isArray(getAllAdmin.data.data?.data)) throw new Error("Expected array in data.data");
    if (!getAllAdmin.data.data?.meta) throw new Error("Expected meta in data.data");
  });
  ...
  assert("Pagination and search query works", () => {
    if (getAllWithQuery.status !== 200) throw new Error(`Expected 200, got ${getAllWithQuery.status}`);
    if (!getAllWithQuery.data.data?.meta?.page) throw new Error("Expected meta.page");
    if (!getAllWithQuery.data.data?.meta?.limit) throw new Error("Expected meta.limit");
  });

  const getEmployeeList = await request("GET", "/products", undefined, employeeToken);
  assert("Employee can get all products (200)", () => {
    if (getEmployeeList.status !== 200) throw new Error(`Expected 200, got ${getEmployeeList.status}`);
  });
  ...
  assert("get single product (200)", () => {
    if (getSingleAdmin.status !== 200) throw new Error(`Expected 200, got ${getSingleAdmin.status} - ${JSON.stringify(getSingleAdmin.data)}`);
    if (!getSingleAdmin.data.data?.name) throw new Error("Expected product data");
  });
  ...
  assert("get single product with invalid ID returns 404", () => {
    if (getInvalidId.status !== 404) throw new Error(`Expected 404, got ${getInvalidId.status}`);
  });
  ...
  assert("Employee can get single product by ID (200)", () => {
    if (getEmployeeSingle.status !== 200) throw new Error(`Expected 200, got ${getEmployeeSingle.status}`);
  });
  ...
  assert("Admin can update product with new image (200)", () => {
    if (updateAdmin.status !== 200) throw new Error(`Expected 200, got ${updateAdmin.status} - ${JSON.stringify(updateAdmin.data)}`);
    if (updateAdmin.data.data?.name !== "Updated Laptop Name") throw new Error("Name not updated");
  });
  ...
  assert("Admin can update product without new image", () => {
    if (updateBodyOnly.status !== 200) throw new Error(`Expected 200, got ${updateBodyOnly.status}`);
    if (updateBodyOnly.data.data?.sellingPrice !== 1700) throw new Error("Price not updated");
  });
  ...
  assert("Manager can update product (200)", () => {
    if (managerUpdate.status !== 200) throw new Error(`Expected 200, got ${managerUpdate.status}`);
  });
  ...
  assert("Employee cannot update product (403)", () => {
    if (employeeUpdate.status !== 403) throw new Error(`Expected 403, got ${employeeUpdate.status}`);
  });
  ...
  assert("update with invalid ID returns 404", () => {
    if (updateInvalidId.status !== 404) throw new Error(`Expected 404, got ${updateInvalidId.status}`);
  });
  ...
  assert("Admin can soft delete product (200)", () => {
    if (deleteAdmin.status !== 200) throw new Error(`Expected 200, got ${deleteAdmin.status} - ${JSON.stringify(deleteAdmin.data)}`);
    if (deleteAdmin.data.data?.isDeleted !== true) throw new Error("Expected isDeleted true");
  });
  ...
  assert("Manager cannot delete product (403)", () => {
    if (managerDelete.status !== 403) throw new Error(`Expected 403, got ${managerDelete.status}`);
  });
  ...
  assert("Employee cannot delete product (403)", () => {
    if (employeeDelete.status !== 403) throw new Error(`Expected 403, got ${employeeDelete.status}`);
  });
  ...
  assert("delete with invalid ID returns 404", () => {
    if (deleteInvalidId.status !== 404) throw new Error(`Expected 404, got ${deleteInvalidId.status}`);
  });
  ...
  assert("soft deleted product is not visible", () => {
    if (!Array.isArray(allAfterDelete.data.data?.data)) throw new Error("Expected array in data.data");
    const found = allAfterDelete.data.data.data.find((p: any) => p._id === createdProductId);
    if (found) throw new Error("Soft deleted product still visible");
  });
