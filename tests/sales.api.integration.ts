...
const run = async () => {
  ...
  const salePayload = (productId: string, quantity: number, sellingPrice: number) => ({
  ...
  assert("Admin can create sale (201)", () => {
    if (createAdminSale.status !== 201) throw new Error(`Expected 201, got ${createAdminSale.status} - ${JSON.stringify(createAdminSale.data)}`);
    if (!createAdminSale.data.data?.grandTotal) throw new Error("Missing grandTotal");
  });
  ...
  assert("Manager can create sale (201)", () => {
    if (createManagerSale.status !== 201) throw new Error(`Expected 201, got ${createManagerSale.status} - ${JSON.stringify(createManagerSale.data)}`);
  });
  ...
  assert("Employee can create sale (201)", () => {
    if (createEmployeeSale.status !== 201) throw new Error(`Expected 201, got ${createEmployeeSale.status} - ${JSON.stringify(createEmployeeSale.data)}`);
  });
  ...
  assert("create sale with missing items returns 400/500", () => {
    if (createMissingProducts.status < 400) throw new Error(`Expected error status, got ${createMissingProducts.status}`);
  });
  ...
  assert("create sale with invalid product id returns 404", () => {
    if (createInvalidProductId.status !== 404) throw new Error(`Expected 404, got ${createInvalidProductId.status}`);
  });

  const createEmptyProducts = await request("POST", "/sales", { items: [] }, adminToken);
  assert("create sale with empty items returns error", () => {
    if (createEmptyProducts.status < 400) throw new Error(`Expected error status, got ${createEmptyProducts.status}`);
  });
  ...
  assert("verified product exists before sale", () => {
    if (productBefore.status !== 200) throw new Error(`Expected 200, got ${productBefore.status}`);
  });
  ...
  assert("helper sale for stock verification (201)", () => {
    if (helperSale.status !== 201) throw new Error(`Expected 201, got ${helperSale.status}`);
  });
  ...
  assert("stock decreased by sale quantity", () => {
    if (productAfter.status !== 200) throw new Error(`Expected 200, got ${productAfter.status}`);
    const remaining = productAfter.data.data?.stockQuantity;
    if (remaining !== (initialStock - 1)) throw new Error(`Expected stock ${initialStock - 1}, got ${remaining}`);
  });
  ...
  assert("Admin can create sale with multiple items (201)", () => {
    if (multiSale.status !== 201) throw new Error(`Expected 201, got ${multiSale.status} - ${JSON.stringify(multiSale.data)}`);
    const expectedTotal = 3 * 400 + 2 * 300;
    if (multiSale.data.data?.grandTotal !== expectedTotal) throw new Error(`Expected grandTotal ${expectedTotal}, got ${multiSale.data.data?.grandTotal}`);
  });
