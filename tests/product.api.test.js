...
const run = () => {
    ...
    assert("returns default filter with isDeleted false", () => {
        ...
        if (result.filter.isDeleted !== false)
            throw new Error("Expected isDeleted false");
    });
    assert("filters by name case-insensitive regex", () => {
        const result = buildProductQuery({ name: "laptop" });
        if (!result.filter.name || !result.filter.name.$regex)
            throw new Error("Expected name filter");
        if (result.filter.name.$regex !== "laptop")
            throw new Error("Expected laptop");
        if (result.filter.name.$options !== "i")
            throw new Error("Expected case-insensitive");
    });
    assert("filters by sku", () => {
        const result = buildProductQuery({ sku: "SKU-001" });
        if (!result.filter.sku)
            throw new Error("Expected sku filter");
    });
    assert("filters by category", () => {
        const result = buildProductQuery({ category: "Electronics" });
        if (!result.filter.category)
            throw new Error("Expected category filter");
    });
    assert("defaults sortBy to createdAt", () => {
        const result = buildProductQuery({});
        if (!result.sort.createdAt)
            throw new Error("Expected createdAt sort");
        if (result.sort.createdAt !== 1)
            throw new Error("Expected ascending");
    });
    assert("sorts desc when sortOrder is desc", () => {
        const result = buildProductQuery({ sortBy: "price", sortOrder: "desc" });
        if (result.sort.price !== -1)
            throw new Error("Expected descending");
    });
    ...
    assert("defaults limit to 10", () => {
        const result = buildProductQuery({});
        if (result.limit !== 10)
            throw new Error("Expected limit 10");
    });
    assert("respects custom page and limit", () => {
        const result = buildProductQuery({ page: 3, limit: 20 });
        if (result.page !== 3)
            throw new Error("Expected page 3");
        if (result.limit !== 20)
            throw new Error("Expected limit 20");
        if (result.skip !== 40)
            throw new Error("Expected skip 40");
    });
    assert("ignores invalid page (string) and defaults to 1", () => {
        const result = buildProductQuery({ page: "abc" });
        if (result.page !== 1)
            throw new Error("Expected page 1");
    });
    ...
    assert("product.route.ts exports productRouter", async () => {
        ...
        if (!mod.productRouter)
            throw new Error("Missing productRouter");
    });
    ...
    assert("GET / uses auth with Admin, Manager, Employee", async () => {
        ...
        const layer = productRouter.stack.find((l) => l.route && l.route.methods.get && l.route.path === "/");
        if (!layer)
            throw new Error("Route not found");
        const middlewares = layer.route.stack.map((m) => m.handle.name || m.handle);
        if (!middlewares.includes("auth"))
            throw new Error("Missing auth middleware");
    });
    assert("POST / uses upload.single('image')", async () => {
        const { productRouter } = await import("../src/modules/product/product.route.js");
        const layer = productRouter.stack.find((l) => l.route && l.route.methods.post && l.route.path === "/");
        if (!layer)
            throw new Error("Route not found");
        const hasUpload = layer.route.stack.some((m) => m.handle.name === "upload");
        if (!hasUpload)
            throw new Error("Missing upload middleware");
    });
    assert("PATCH /:id uses auth and upload", async () => {
        const { productRouter } = await import("../src/modules/product/product.route.js");
        const layer = productRouter.stack.find((l) => l.route && l.route.methods.patch && l.route.path === "/:id");
        if (!layer)
            throw new Error("Route not found");
        const middlewares = layer.route.stack.map((m) => m.handle.name || m.handle);
        if (!middlewares.includes("auth"))
    ...
    assert("DELETE /:id uses auth", async () => {
        const { productRouter } = await import("../src/modules/product/product.route.js");
        const layer = productRouter.stack.find((l) => l.route && l.route.methods.delete && l.route.path === "/:id");
        if (!layer)
            throw new Error("Route not found");
        const middlewares = layer.route.stack.map((m) => m.handle.name || m.handle);
        if (!middlewares.includes("auth"))
    ...
    assert("product.interface.ts exports IProduct", async () => {
        ...
        if (!mod.IProduct)
            throw new Error("Missing IProduct");
    });
    assert("IProduct has required fields", async () => {
        ...
        for (const field of fields) {
            if (!(field in IProduct.prototype))
