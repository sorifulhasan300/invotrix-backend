...
export const registerUser = async (data: RegisterInput) => {
  ...
  return { success: true, statusCode: StatusCodes.CREATED, message: "User registered successfully", data: userObject };
};
...
export const loginUser = async (data: LoginInput) => {
  ...
  const payload = { id: user._id.toString(), role: user.role as "Admin" | "Manager" | "Employee" };
  const token = jwt.sign(payload, process.env.JWT_SECRET as string, { expiresIn: "7d" });

  const userObject = user.toObject();
  delete (userObject as any).password;

  return { success: true, statusCode: StatusCodes.OK, message: "Login successful", data: { user: userObject, token } };
};
